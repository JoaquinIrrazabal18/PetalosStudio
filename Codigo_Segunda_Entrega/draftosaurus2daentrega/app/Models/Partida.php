...
class Partida {
    ...
    public function crear(array $data): array {
        try {
            ...
            // Validaciones según EsRe
            if (!in_array($lado_tablero, ['Verano', 'Invierno'])) {
                $lado_tablero = 'Verano';
            }

            if (!is_array($jugadores) || empty($jugadores)) {
                return ['success' => false, 'message' => 'Se requieren jugadores para crear la partida'];
            }

            $num_participantes = count($jugadores);
            if ($num_participantes < 1 || $num_participantes > 5) {
                return ['success' => false, 'message' => 'La partida debe tener entre 1 y 5 jugadores'];
            }

            // RFD1: Configurar dinosaurios según número de participantes
            $dinos_iniciales = $this->calcularDinosauriosBolsa($num_participantes);
            ...
            return ['success' => false, 'message' => 'Error interno al crear la partida'];
        }
    }
